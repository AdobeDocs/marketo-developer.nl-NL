...
description: requestCampaign SOAP-oproepen
...
| --- | --- | --- |
| leadList->leadKey->keyType | Vereist | In `keyType` kunt u het veld opgeven waarop u een query wilt uitvoeren. Mogelijke waarden zijn: `IDNUM`, `EMAIL`, `SFDCLEADID`, `LEADOWNEREMAIL`, `SFDCACCOUNTID`, `SFDCCONTACTID`, `SFDCLEADID`, `SFDCLEADOWNERID`, `SFDCOPPTYID` |
| leadList->leadKey->keyValue | Vereist | `keyValue` is de waarde waardoor u de lead wilt opvragen. |
| bron | Vereist | De campagnebron. Mogelijke waarden: `MKTOWS` of `SALES` . Opsomming wordt gedefinieerd in WSDL. |
| campagneId | Optioneel wanneer `campaignName` , `programName` en `programTokenList` zich op een parametersite bevinden; anders is `campaignId` vereist | De id van de campagne. OPMERKING: er treedt een ongeldige parameterfout op als `campaignID` en `campaignName` beide worden doorgegeven. |
