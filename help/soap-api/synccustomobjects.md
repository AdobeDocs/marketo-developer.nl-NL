...
feature: SOAP
description: Gebruik Marketo syncCustomObjects SOAP om maximaal 100 aangepaste objecten per aanroep in te voegen, bij te werken of bij te voegen met statusresultaten per record en XML- en PHP-voorbeelden.
...
Accepteert een array van aangepaste objecten die moeten worden gemaakt of bijgewerkt, tot maximaal 100 per aanroep. Het retourneert het resultaat (status) van de bewerking (GEMAAKT, BIJGEWERKT, MISLUKT, ONGEWIJZIGD, OVERGESLAGEN) en de aangepaste objecten die zijn verwerkt. De API wordt aangeroepen in een van de drie bewerkingsmodi:
...
In één enkele API vraag, sommige updates kunnen slagen en sommige kunnen ontbreken. Voor elke fout wordt een foutbericht geretourneerd.

Voor aangepaste objecten die zijn ingericht met de nieuwe gebruikersinterface voor aangepaste objecten, kunnen alleen velden die zijn aangewezen als `dedupe` -velden in de gebruikersinterface als kenmerken worden doorgegeven in `CustomObjKeyList` . Koppelingsvelden die geen `dedupe` -velden zijn, moeten als kenmerk worden doorgegeven in `customObjAttributeList` .

## Verzoek

| Veldnaam | Vereist/optioneel | Beschrijving |
| --- | --- | --- |
| bewerking | Vereist | &quot;INSERT&quot;, &quot;UPDATE&quot; of &quot;UPSERT&quot; |
| objectTypeName | Vereist | Naam van het aangepaste object |
| customObjectList->customObject->customObjKeyList->attribute | Vereist | Het kenmerk is een sleutel-/waardepaar waarmee het aangepaste object wordt geïdentificeerd. U kunt meerdere kenmerken hebben in de customObjKeyList |
| customObjectList->customObject->customObjAttributeList->attribute | Vereist | Sleutel/waardeparen, waarbij de naam de gebiedsnaam en de waarde de waarde is die u in customObject wilt opnemen |
