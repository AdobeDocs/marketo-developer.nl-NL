...
Deze API is vergelijkbaar `getLeadActivity` behalve dat het op veelvoudige lood in één keer werkt. De bewerking controleert op nieuwe leads die zijn gemaakt, updates voor leidende velden en andere activiteiten.

Het resultaat bevat activiteiten die de wijziging samen met een [streampositie](stream-position.md) om door grote resultaatreeksen te pagineren.
...
Na SOAP API versie 2_2 kunt u een `leadSelector`.

Voor `LastUpdateAtSelector`de `oldestUpdatedAt` waarde zou overeenkomen met `oldestCreatedAt` waarde in de `startPosition`. En de `latestUpdatedAt` waarde zou overeenkomen met de `latestCreatedAt` waarde in de `startPosition`.

Opmerking: het limietaantal leads dat wordt ondersteund in een `LeadKeySelector` is 100. Als het aantal leads groter is dan 100, genereert de API een ongeldige parameteruitzondering en retourneert deze een SOAP-fout.

## Verzoek

| Veldnaam | Vereist/optioneel | Beschrijving |
| --- | --- | --- |
| activityFilter->includeAttributes->activityType | Optioneel (vervangen) gebruik `activityNameFilter` in | Beperkt de reactie om slechts die gespecificeerde activiteitstypes op te nemen. Zie WSDL voor alle activiteitstypen. |
| activityFilter->excludeAttributes->activityType | Optioneel | Hiermee wordt de reactie beperkt om de opgegeven activiteitstypen uit te sluiten. Zie WSDL voor alle activiteitstypen. OPMERKING: u kunt niet beide opgeven `includeAttributes` en `excludeAttributes` binnen de zelfde vraag. |
| activityNameFilter | Optioneel | Beperkt de reactie zodat alleen de opgegeven activiteitsfilters worden opgenomen. |
| batchSize | Optioneel | Maximumaantal records dat moet worden geretourneerd. Systeem beperkt tot 1.000 of `batchSize`, afhankelijk van welke waarde lager is. |
...
| startPosition->oldestCreatedAt | Optioneel | De tijdstempel die wordt gebruikt om resultaten te filteren, resulteert alleen in leads die zijn gemaakt sinds de oudsteCreatedAt. OPMERKING: u kunt `LastUpdateAtSelector->oldestUpdatedAt` te specificeren tijdstempel `oldestCreatedAt`. |
| startPosition->activityCreatedAt | Optioneel | De tijdstempel die wordt gebruikt om resultaten te filteren, resulteert alleen in het opnemen van leads met activiteit sinds activityCreatedAt. OPMERKING: u kunt `LastUpdateAtSelector->latestUpdatedAt` te specificeren tijdstempel `activityCreatedAt`. |
| leadSelector | Optioneel | Kan een van de volgende drie typen zijn: `LeadKeySelector`, `StaticListSelector`, `LastUpdateAtSelector` |
| LeadKeySelector: leadSelector->keyType | Vereist | Het id-type waarop u een query wilt uitvoeren. Waarden omvatten `IDNUM`, `COOKIE`, `EMAIL`, `LEADOWNEREMAIL`, `SFDCACCOUNTID`, `SFDCCONTACTID`, `SFDCLEADID`, `SFDCLEADOWNERID`, `SFDCOPPTYID`. |
| LeadKeySelector: leadSelector->keyValues->stringItem | Vereist | Lijst met sleutelwaarden. Dat is &quot;lead@email.com&quot; |
| StaticListSelector: leadSelector->staticListName | Optioneel indien `leadSelector->staticListId` is aanwezig | De naam van de statische lijst |
| StaticListSelector: leadSelector->staticListId | Optioneel indien `leadSelector->staticListName` is aanwezig | De id van de statische lijst |
