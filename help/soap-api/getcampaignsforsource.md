...
Deze functie retourneert een lijst met in aanmerking komende Marketo-campagnes die als invoerparameters in de requestCampaign-functie kunnen worden gebruikt. Campagnes worden gecategoriseerd door de bron van het verzoek, dat in WSDL wordt geteld.
...
| --- | --- | --- |
| bron | Vereist | De bron kan `MKTOWS` of `SALES` zijn. Deze laatste biedt een lijst met campagnes die beschikbaar zijn voor Sales Insight. |
| name | Optioneel | gebruik dit om op naam te filteren. Dit is één tekenreeks, geen array van tekenreeksen. |
| exactName | Optioneel | Booleaanse waarde die aangeeft of de parameter name exact moet overeenkomen |
