...
feature: SOAP
description: Gebruik getImportToListStatus om de status van de Marketo statische lijst van de invoer, vereiste gebieden, het verzoek en de reactie van XML, PHP en code Java, en opiniepeilend beste praktijken te controleren.
...
Deze functie controleert de status van een specifieke `importToList` functieaanroep.

Voltooi eerst de API-aanroep van `importToList` en bekijk vervolgens periodiek `getImportToListStatus` . Niet meer dan één keer per minuut opiniepeilen `getImportToListStatus` .
...
| listName | Vereist | Naam van de statische lijst in Marketo waarvoor de importstatus moet worden opgehaald. |
